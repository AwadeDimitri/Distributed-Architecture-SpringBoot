...
import com.univkara.collectivities.dto.MarcheDto;
...
public class MarcheController {
	...
	public ResponseEntity<List<Marche>> getAllMarches(){
		try {
			List<Marche> marches = marcheRepository.findAll();
			if(marches.isEmpty()) {
	...
	//create market
	@PostMapping("/marches")
	public ResponseEntity<MarcheDto> createMarche(@RequestBody MarcheDto marchedto) {
		try {
		
			Marche my_marche = new Marche ();
			my_marche.setNom(marchedto.getNom());
			my_marche.setEmplacement(marchedto.getEmplacement());
			my_marche.setSuperficie(marchedto.getSuperficie());
			
			Marche _marche = marcheRepository
					.save(my_marche);
			
			MarcheDto returnMarche = new MarcheDto();
			returnMarche.setNom(_marche.getNom());
			returnMarche.setSuperficie(_marche.getSuperficie());
			returnMarche.setEmplacement(_marche.getEmplacement());
			
			return new ResponseEntity<>(returnMarche, HttpStatus.CREATED);
		} catch (Exception e) {
			return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
